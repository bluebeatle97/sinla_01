...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderRightContent() {
    ...
    megaRightContainer.innerHTML =
        `<div class="promo-grid">
            ${promotions.map((item) => 
            `<a href="#" class="promo-item"><img src="${item.imageUrl}" alt="${item.name}"><p>${item.name}</p></a>`).join("")}</div>
  ...
  function renderBrands(filter = "") {
    ...
    let targetKeys =
      currentSort === "kor"
        ? allKeys.filter((k) => /[ㄱ-ㅎ가-힣]/.test(k) && k !== "기타").sort()
        : allKeys.filter((k) => /[A-Z]/.test(k)).sort();
    ...
    targetKeys.forEach((key) => {
      const brands = brandData[key].filter((b) =>
      ...
      indexBtn.addEventListener("click", () => {
        ...
        if (targetEl)
          document
            .querySelector(".brand-list-scroll")
            .scrollTo({ top: targetEl.offsetTop, behavior: "smooth" });
      });
  ...
  if (iconsList) {
    fetch("main_icons.json")
      .then((r) => r.json())
      .then((data) => {
        iconsList.innerHTML = data.main_icons
          .map( (icon) => `<li class="icon-item">
  ...
  const exchangeRate = 1496.0;

  if (moduleList) {
    fetch("products.json")
      .then((response) => response.json())
      .then((data) => {
        const productsWithIdx = data.products.map((p, i) => ({
          ...p,
          originalIdx: i,
        }));
        const shuffledProducts = productsWithIdx.sort(
          () => 0.5 - Math.random(),
        );
        const products = shuffledProducts.slice(0, 10);

        moduleList.innerHTML = products
          .map((product) => {
  ...
  function renderModuleProducts5(container, products) {
    container.innerHTML = products
      .map((product) => {
        ...
        const originalUsd = parseFloat(product.price_usd);
        const finalUsd = Math.floor(originalUsd * (1 - discount / 100));
        const finalKrw = Math.floor(finalUsd * 1496.0);
  ...
  setupModule5("module-products-5-1", (p) =>
    ["스킨케어", "메이크업", "향수/바디/헤어"].includes(p.category),
  );
  setupModule5("module-products-5-2", (p) =>
    ["가방/신발/잡화", "시계/쥬얼리", "의류/아이웨어"].includes(p.category),
  );
  ...
  setupModule5(
    ...
    (products) => {
      return products
        .sort(
          (a, b) =>
            parseInt(b.review_count || 0) - parseInt(a.review_count || 0),
        )
        .slice(0, 15);
    },
  );
  ...
  if (eventList) {
    fetch("brand_event.json")
      .then((response) => response.json())
      .then((data) => {
        eventList.innerHTML = data.brand_event
          .map(
            (event) => `
  ...
  if (btnEventNext) {
    btnEventNext.addEventListener("click", () => {
      ...
      eventScrollPos = Math.min(
        eventScrollPos + eventItemWidth,
        maxScrollEvent,
      );
  ...
  function renderBenefits(type) {
    ...
    let filtered =
      type === "shopping" ? benefitData.slice(0, 2) : benefitData.slice(2, 8);
    benefitList.innerHTML = filtered
      .map(
        (item) => `
            <li class="benefit-item">
                <div class="benefit-card">
                    <div class="img-box"><img src="${item.file}" alt="${item.title}"></div>
                    <div class="info-box"><p class="title">${item.title}</p>
                    <p class="desc">${item.content}</p></div>
                </div>
            </li>
        `,
      )
  ...
  function startBenefitAutoSlide() {
    benefitInterval = setInterval(() => {
      ...
      benefitScrollPos =
        benefitScrollPos >= maxScroll ? 0 : benefitScrollPos + itemWidth;
  ...
  const setupSlider = (containerSelector, prevSelector, nextSelector) => {
    ...
    const updateTransform = () => {
      list.style.transform = `translateX(-${list.dataset.pos}px)`;
    };
  ...
  const resetSpecial = setupSlider(
    ".special-product-list",
    "#tab-special .btn-prev",
    "#tab-special .btn-next",
  );
  const resetHotSale = setupSlider(
    ".hotsale-product-list",
    "#tab-hotsale .btn-prev",
    "#tab-hotsale .btn-next",
  );
  ...
  function initHotSale() {
    ...
    categoryBtnWrapper.innerHTML = categories
      .map(
        (cat, idx) =>
          `<button type="button" class="btn-category 
        ${idx === 0 ? "active" : ""}" data-category="${cat}">${cat}
        </button>`,
      )
      .join("");
    renderHotSale("추천");
  ...
  function renderHotSale(category) {
    let filtered =
      category === "추천"
        ? [...allProducts].sort(() => 0.5 - Math.random()).slice(0, 12)
  ...
  function startTimer() {
    ...
    setInterval(() => {
      const now = new Date();
      timerEl.textContent = 
      `${String(23 - now.getHours()).padStart(2, "0")}:
  ...
  shillaTabBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".shilla-tab-content")
        .forEach((c) => c.classList.remove("active"));
      document
        .getElementById(`shilla-tab-${btn.dataset.tab}`)
        .classList.add("active");

      if (btn.dataset.tab === "product") resetShillaProduct();
      else resetShillaBrand();
    });
  });
  ...
  fetch("products.json")
    ...
    .then((data) => {
      ...
      const shillaOnlyProducts = productsWithIdx.filter((p) =>
        p.name.includes("[신라단독]"),
      );
      renderShillaProducts(shillaOnlyProducts);
    });

  // 신라 Only 브랜드 데이터 로드
  fetch("shinlla_only_brand.json")
    .then((r) => r.json())
    .then((data) => {
  ...
  function renderShillaProducts(products) {
    ...
    shillaProductList.innerHTML = products
      .map((product) => {
        const discount = parseInt(product.price_discount);
        const originalUsd = parseFloat(product.price_usd);
        const finalUsd = Math.floor(originalUsd * (1 - discount / 100));
  ...
  function renderShillaBrands(brands) {
    ...
    shillaBrandList.innerHTML = brands
      .map(
        (brand) => `
  ...
  const resetShillaProduct = setupShillaSlider(
    ".shilla-product-list",
    "#shilla-tab-product .btn-prev",
    "#shilla-tab-product .btn-next",
    325,
  );
  const resetShillaBrand = setupShillaSlider(
    ".shilla-brand-list",
    "#shilla-tab-brand .btn-prev",
    "#shilla-tab-brand .btn-next",
    433,
  );
  ...
  function renderPopularSearches() {
    ...
    popularRankList.innerHTML = popularSearches
      .map(
        (text, idx) => `
            <li>
  ...
  if (productDetailSection) {
    fetch("products.json")
      ...
      .then((data) => {
        ...
        if (categoryPath)
          categoryPath.innerHTML = `홈 &gt; ${product.category} &gt; ${product.sub_category}`;
        if (productImg) {productImg.src = product.img; productImg.alt = product.name;}
        ...
        if (discount > 0) {
          ...
          if (originalUsdEl)
            originalUsdEl.textContent = `$${originalUsd.toLocaleString()}`;
          if (originalKrwEl)
            originalKrwEl.textContent = `(${originalKrw.toLocaleString()}원)`;
        } else {
        ...
        if (finalPrice)
          finalPrice.textContent = `$${calcFinalUsd.toLocaleString()}`;
        if (wonPrice)
          wonPrice.textContent = `(${calcFinalKrw.toLocaleString()}원)`;
        if (totalPrice)
          totalPrice.textContent = `$${calcFinalUsd.toLocaleString()}`;
